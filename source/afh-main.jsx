// afh-main.jsx — App shell + routing
...
const App = () => {
  ...
  const [collapsed, setCollapsed] = useState(() => localStorage.getItem('afh-sidebar') === '1');
  const [cmdOpen, setCmdOpen] = useState(false);
  const [selectedRun, setSelectedRun] = useState(null);

  // Persist settings
  useEffect(() => { localStorage.setItem('afh-theme', theme); }, [theme]);
  useEffect(() => { localStorage.setItem('afh-sidebar', collapsed ? '1' : '0'); }, [collapsed]);
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: 'var(--bg-base)', color: 'var(--text-primary)', fontFamily: 'Geist Sans, -apple-system, BlinkMacSystemFont, sans-serif', fontVariantNumeric: 'tabular-nums' }}>
      <Sidebar page={page} setPage={p => { setPage(p); setSelectedRun(null); }} collapsed={collapsed} setCollapsed={setCollapsed} />
      ...
      <CommandPalette open={cmdOpen} onClose={() => setCmdOpen(false)} setPage={p => { setPage(p); setSelectedRun(null); }} />
    </div>
  );
};
